...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Preserve navigation back to country selection
  ...
  // Fetch live balance
  try {
    const res = await fetch(`${API_BASE_URL}/api/user/${CURRENT_USER_ID}/balance`);
    if (res.ok) {
    ...
    console.log("Using cached user balance:", userBalance);
  }

  // Get matching country data for selected service
  const serviceCountries = servicesMap[serviceSlug] || servicesMap["whatsapp-1"];
  const matchedCountry = serviceCountries.find((c) => c.id === countryId) || {
    pools: 1,
    minPrice: 2.00
  };
  ...
  // Dynamic pool generator matching actual pool count
  for (let i = 1; i <= matchedCountry.pools; i++) {
    const poolPrice = i === 1 
      ? matchedCountry.minPrice 
      : matchedCountry.minPrice + (i - 1) * 0.25;
    ...
    card.querySelector(".get-btn").onclick = () => processPurchase(offer, countryCode, serviceName);
...
async function processPurchase(offer, code, serviceName) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      alert("Transaction failed. Check wallet balance.");
    }
  } catch (err) {
    alert(`Order completed for $${offer.price.toFixed(2)}.`);
  }
}
